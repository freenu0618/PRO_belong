...
echo "?îÑ Starting Database Restore from: $BACKUP_DIR"

# 1. Restore ChromaDB
echo "?ß† Restoring ChromaDB..."
...
if [ -d "$CHROMA_BACKUP" ]; then
    ...
    echo "?†Ô∏è No ChromaDB backup found, skipping..."
fi

# 2. Restore PostgreSQL
echo "?êò Restoring PostgreSQL..."
...
if [ -f "$PG_BACKUP" ]; then
    ...
    pg_restore -h 127.0.0.1 -U belong -d belong_db "$PG_BACKUP" || {
        echo "?†Ô∏è pg_restore had some warnings (this is often normal)"
    }
    echo "??PostgreSQL restored"
else
    echo "?†Ô∏è No PostgreSQL backup found, skipping..."
fi

# 3. Restore Fine-tuned Models
echo "?§ñ Restoring Fine-tuned models..."
...
if [ -d "$MODELS_BACKUP" ]; then
    ...
    echo "?†Ô∏è No models backup found, skipping..."
fi

echo ""
echo "??Restore Complete!"
echo "?í° Restart the application to apply changes:"
